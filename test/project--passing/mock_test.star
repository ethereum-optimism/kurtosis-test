...
def test_mock_non_builtin(plan):
    mock_module_function = kurtosistest.mock(mock_test_module, "module_function").mock_return_value(16)
...
def test_mock_non_existing(plan):
    assert.fails(lambda: kurtosistest.mock(mock_test_module, "non_existing_function"), "doesn't have a property called non_existing_function")

def test_mock_non_function(plan):
    assert.fails(lambda: kurtosistest.mock(mock_test_module, "module_constant"), "is not a function, it's {0}".format(mock_test_module.module_constant))

def test_mock_non_module(plan):
    target = struct()
    assert.fails(lambda: kurtosistest.mock(target, "something"), "mock: only module mocks are possible at the moment. struct\\(\\) is not a module")
